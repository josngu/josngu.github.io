...
export async function createTransition(message, color, duration) {
    try {
...
export async function createDialogBox(message, color) {
...
export function log(message, accentColor) {
...
export function getAnimationDuration(className) {
...
export function updatePlayerClass(playerNumber) {
...
export function updateVisualPlayerStats(updateAllPlayers) {
    if (updateAllPlayers === true) {
        for (let i = 1; i <= gameState.playerList.length; i++) {
            ...
            // Update the WP orbs
            let filledOrbs = document.getElementById(`player-wp-current-${i}`).getElementsByClassName('wp-orb-filled');
            ...
            if (filledOrbs.length < player.currentWp) {
                while (filledOrbs.length != player.currentWp) {
                    try {
                        ...
                        document.getElementById(`player-wp-current-${i}`).insertAdjacentHTML('afterbegin', '<span class="wp-orb-filled"></span>');
                    }
                    document.getElementById(`player-wp-current-${i}`).getElementsByClassName('wp-orb-empty')[0].remove();
                }
            }
            ...
            if (filledOrbs.length > player.currentWp) {
                while (filledOrbs.length != player.currentWp) {
                    document.getElementById(`player-wp-current-${i}`).getElementsByClassName('wp-orb-filled')[0].remove();
                    document.getElementById(`player-wp-current-${i}`).insertAdjacentHTML('beforeend', '<span class="wp-orb-empty"></span>');
                    // Update the filledOrbs variable
                    filledOrbs = document.getElementById(`player-wp-current-${i}`).getElementsByClassName('wp-orb-filled');
                }
            }
        }
    }
...
export function checkStatBounds(checkAllPlayers) {
    ...
    if (checkAllPlayers === true) {
        for (let i = 0; i < gameState.playerList.length; i++) {
            if (PLAYER_LIST[i].currentHp > PLAYER_LIST[i].maxHp)
                PLAYER_LIST[i].currentHp = PLAYER_LIST[i].maxHp;
            if (PLAYER_LIST[i].currentWp > PLAYER_LIST[i].maxWp)
                PLAYER_LIST[i].currentWp = PLAYER_LIST[i].maxWp;
            if (PLAYER_LIST[i].currentHp <= 0)
        ...
        const CURRENT_PLAYER = getCurrentPlayer();
        if (CURRENT_PLAYER.currentHp > CURRENT_PLAYER.maxHp)
            CURRENT_PLAYER.currentHp = CURRENT_PLAYER.maxHp;
        if (CURRENT_PLAYER.currentWp > CURRENT_PLAYER.maxWp)
            CURRENT_PLAYER.currentWp = CURRENT_PLAYER.maxWp;
        if (CURRENT_PLAYER.currentHp <= 0)
            CURRENT_PLAYER.currentHp = 0;
