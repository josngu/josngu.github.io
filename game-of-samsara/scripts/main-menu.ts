...
export function createPlayers() {
    for (let i = 1; i <= game.gameState.numberOfPlayers; i++) {
        let playerName = (document.getElementById(`player-${i}`) as HTMLInputElement).value;
        if (playerName == '') {
            playerName = `Player ${i}`;
        }
        let playerColor = (document.getElementById(`player-color-${i}`) as HTMLInputElement).value;

        game.gameState.playerList.push({
            playerName: playerName,
            hexColor: playerColor,
            darkenedRGBColor: game.convertHexToRGBArray(game.darkenHexColor(playerColor, 80)) as [number, number, number],
            class: 'Pariah',
            progenitor: '',
            level: 1,
            lives: 9,
...
export function showHoveredProgenitor() {
    ...
    const PROGENITOR_THUMBNAIL = (this as HTMLElement).querySelector('img');
...
export function selectProgenitor() {
    ...
    const PROGENITOR_NAME = (this as HTMLElement).querySelector('img').getAttribute('alt');
    const PROGENITOR_IMAGE_SOURCE = (this as HTMLElement).querySelector('img').getAttribute('src');
...
function progenitorScreenSelectPlayer() {
    ...
    const playerCard = this as HTMLElement;
